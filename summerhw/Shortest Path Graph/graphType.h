...
public:
    ...
    void setSize(int size);   // Declare the setSize function
...
void graphType::setSize(int size)
{
if (size < 0)
    {
    ...
    // Reallocate memory for the graph array
    delete[] graph;
    graph = new unorderedLinkedList<int>[size];
}

void graphType::addEdge(int fromVertex, int toVertex)
{
 if (fromVertex < 0 || fromVertex >= gSize || toVertex < 0 || toVertex >= gSize)
    {
        std::cerr << "Error: Invalid vertex index." << std::endl;
        return;
    }

    graph[fromVertex].insertLast(toVertex);
}
