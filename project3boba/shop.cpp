...
bool Shop::SellItem(int index, int player_money) //selling items to player method
{
    if (index < 0 || index >= menu_.size() || player_money < prices_[index]) return false; //case if item doesn't exist or player can't afford item

    player_money -= prices_[index]; //subtract price from balance of player
