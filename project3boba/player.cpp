...
void Player::displayInventory() //displays inventory
{
    ...
    else //case if inventory is not empty
    {
        ...
        for (int i = 0; i < inventory_.size(); i++) //for loop to go through inventory
        {
            cout << "- " << inventory_[i] << endl; //print item
        }
    }
}
...
void Player::drinkBeverage() //lets player drink boba
{
    if (inventory_.empty()) //case if inventory is empty
    {
        cout << "Inventory is empty." << endl;
    ...
    else //case if inventory is not empty
    {
        for (int i = 0; i < inventory_.size(); i++) //for loop to go through inventory
        {
            if (inventory_[i] == "boba") //case if boba is in inventory
            {
                inventory_.erase(inventory_.begin() + i); //remove boba from inventory
                thirst_ -= 50; //subtract 50 from thirst
                cout << "You drank boba and your thirst is now " << thirst_ << "." << endl;
                return;
            }
        }
        cout << "You don't have any boba." << endl; //case if boba is not in inventory
    }
}